...
def load_snapshots():
    if os.path.exists(SNAPSHOT_FILE):
        try: st.session_state['snapshots'] = pd.read_csv(SNAPSHOT_FILE).to_dict('records')
        except: st.session_state['snapshots'] = []
    else: st.session_state['snapshots'] = []
def save_snapshot(snap_dict):
    df_new = pd.DataFrame([snap_dict])
    if os.path.exists(SNAPSHOT_FILE): df_new.to_csv(SNAPSHOT_FILE, mode='a', header=False, index=False)
...
def compute_full_analysis(hist_data, spot, gv, td, pv, ivr_val):
    ...
    lv, av = compute_volume_profile(df)
    ...
    align = (spot > sma20) and (sma20 > sma50)
    mb = macd > sig
    ...
    return {
        'adx': adx, 'rsi': rsi, 'macd': macd, 'signal': sig,
        'sma20': sma20, 'sma50': sma50, 'vr': vr, 'regime': regime,
        'bias': bias, 'vol_regime': vr2, 'hurst': hurst,
...
def chart_correlation():
    ...
    ax2.axhline(0.8, color='green', ls='--'); ax2.axhline(0.5, color='red', ls='--')
...
def chart_cone():
    ...
    for w in windows:
        ...
        if not rv.dropna().empty:
            max_v.append(rv.max()); min_v.append(rv.min())
            med_v.append(rv.median()); cur_v.append(rv.iloc[-1])
...
if st.session_state['active_tab'] == "📊 Dashboard":
    ...
    p, i = period_map[tf]
    df_chart = yf_download_retry(ticker, period=p, interval=i)
    ...
    if selected_analysis:
        ...
        st.markdown("---")
        modules = [
            ("Correlation", chart_correlation), ("Expected Move", chart_expected_move),
            ("Hurst Exponent", chart_hurst), ("IV Rank & IV Percentile", chart_ivr),
            ("Liquidity Detector", chart_liquidity), ("Open Interest Profile", chart_oi),
            ("Parkinson Volatility", chart_park), ("Volatility Cone", chart_cone),
    ...
    else: st.warning("No analysis data available. Please refresh.")
    ...
    if prompt := st.chat_input("Ask about the market..."):
        ...
        q = prompt.lower()
        buy_words = ['buy', 'long', 'entry', 'enter', 'purchase']
        sell_words = ['sell', 'short', 'exit', 'dump']
        risk_words = ['risk', 'danger', 'safe', 'volatility', 'stop']
        summary_words = ['summary', 'overview', 'analysis', 'status', 'report']
        if any(w in q for w in buy_words):
            if selected_analysis and selected_analysis['bias'] == 'Bullish':
                ...
                ans = f"⚠️ No clear buy signal. Bias is {selected_analysis.get('bias','Neutral') if selected_analysis else 'Unknown'}. Wait for confirmation."
        elif any(w in q for w in sell_words):
            if selected_analysis and selected_analysis['bias'] == 'Bearish':
                ...
                ans = f"⚠️ No clear sell signal. Market is {selected_analysis.get('regime','uncertain') if selected_analysis else 'unknown'}."
        elif any(w in q for w in risk_words):
            ans = f"🛡️ Risk: GARCH={garch_vol:.1f}%, Daily Move=±{currency}{selected_analysis['daily_move']:,.2f}. IVR={ivr_val:.0f}%. Not advice."
        elif any(w in q for w in summary_words):
            ...
            ans = f"I can help with buy/sell signals, risk assessment, or market summary. Ask away!"
