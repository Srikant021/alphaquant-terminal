# AlphaQuant Terminal — Complete Integration with Habit Tracker & Trading Journal
...
import requests, time, logging, yaml, os, calendar, json
...
<style>
    ...
    .trade-card { background: #1A1D24; border-left: 4px solid #00C48C; padding: 12px; margin: 8px 0; border-radius: 6px; }
    .trade-loss { border-left-color: #FF4D4D; }
    .journal-entry { background: #13161C; border: 1px solid #2A2E39; border-radius: 8px; padding: 16px; margin: 12px 0; }
</style>
...
for key, default in [
    ...
    ('active_tab', '📊 Dashboard'), ('habit_data', pd.DataFrame()), ('trades', []), ('journal_entries', [])
]:
...
class TradingJournal:
    ...
    def add_trade(trade_data):
        trades = TradingJournal.load_journal()
        trade_data['id'] = len(trades) + 1
        trade_data['timestamp'] = datetime.now().isoformat()
        trades.append(trade_data)
        TradingJournal.save_journal(trades)
        return trade_data
    
    @staticmethod
    def add_journal_entry(entry_data):
        entries = TradingJournal.load_journal()
        entry_data['id'] = len(entries) + 1
        entry_data['timestamp'] = datetime.now().isoformat()
        entries.append(entry_data)
        TradingJournal.save_journal(entries)
        return entry_data
    
    @staticmethod
    def get_trades(filter_by=None):
        trades = TradingJournal.load_journal()
...
# CHART FUNCTIONS
# ============================================================================
def chart_correlation(selected_market):
...
def chart_expected_move(hist_data, asset_spot, garch_vol, trading_days, currency):
    iv_use = garch_vol
...
with st.sidebar:
    ...
    st.markdown("---")
    tab_options = ["📊 Dashboard", "📈 Technical", "📓 Habit Tracker", "📝 Trading Journal"]
...
if st.session_state['active_tab'] == "📊 Dashboard":
    ...
    with col_left:
        with st.expander("₿ Crypto Market Overview", expanded=True):
            if btc_analysis:
    ...
    with col_right:
        with st.expander("🇮🇳 Indian Market Overview", expanded=True):
            if nifty_analysis:
    ...
    # Snapshots section
    with st.expander("📸 Daily Snapshots History", expanded=False):
        all_snaps = pd.DataFrame(st.session_state['snapshots'])
        if not all_snaps.empty:
            all_snaps['date'] = pd.to_datetime(all_snaps['date'])
            all_snaps = all_snaps.sort_values('date', ascending=False)
            st.dataframe(all_snaps[['date','asset','spot','garch_vol','ivr']].head(10), use_container_width=True)
        else:
            st.info("No snapshots yet.")

elif st.session_state['active_tab'] == "📈 Technical":
    st.title("📈 Full Technical Analysis Suite")
    st.caption("All technical indicators in one view.")
    ...
    if selected_analysis:
        ...
        with col1:
            st.metric("Market Regime", selected_analysis['regime'], f"Hurst: {selected_analysis['hurst']:.3f}")
            st.metric("Trend Bias", selected_analysis['bias'], f"ADX: {selected_analysis['adx']:.1f}")
        with col2:
            st.metric("RSI (14)", f"{selected_analysis['last_rsi']:.1f}", 
                     "Overbought" if selected_analysis['last_rsi'] > 70 else ("Oversold" if selected_analysis['last_rsi'] < 30 else "Neutral"))
            st.metric("Volatility", selected_analysis['vol_regime'], f"GARCH: {garch_vol:.1f}%")
        with col3:
            st.metric("Expected Daily Move", f"{currency}{selected_analysis['daily_move']:,.2f}")
            st.metric("Volume Ratio", f"{selected_analysis['vol_ratio']:.1f}x")

    # Charts
    col_left, col_right = st.columns(2)
    with col_left:
        st.subheader("Expected Move Chart")
        fig1 = chart_expected_move(hist_data, asset_spot, garch_vol, trading_days, currency)
        if fig1:
            st.pyplot(fig1)
        
        st.subheader("Correlation Analysis")
        fig2 = chart_correlation(selected_market)
        if fig2:
            st.pyplot(fig2)
    ...
    with col_right:
        ...
        if not hist_data.empty:
            fig, ax = plt.subplots(figsize=(10, 5))
            close = hist_data['Close'].squeeze().tail(100)
            sma20 = close.rolling(20).mean()
            sma50 = close.rolling(50).mean()
            ax.plot(close.index, close.values, label='Price', color='white')
            ax.plot(sma20.index, sma20.values, label='SMA 20', color='cyan', linestyle='--')
            ax.plot(sma50.index, sma50.values, label='SMA 50', color='orange', linestyle='--')
            ax.legend()
            ax.set_title(f"{asset_choice} - Price with SMAs")
    ...
    with st.expander("📊 Additional Indicators", expanded=False):
        ...
        with col_a:
            ...
            if not hist_data.empty:
                ...
                rsi = compute_rsi(hist_data['Close'].squeeze())
                ax.plot(rsi.index, rsi.values, color='cyan')
        ...
        with col_b:
            ...
            if not hist_data.empty:
                fig, ax = plt.subplots(figsize=(10, 4))
                macd, signal, hist = compute_macd(hist_data['Close'].squeeze())
                ax.plot(macd.index, macd.values, label='MACD', color='cyan')
                ax.plot(signal.index, signal.values, label='Signal', color='orange')
                ax.bar(hist.index, hist.values, label='Histogram', color='gray', alpha=0.5)
                ax.legend()
                ax.set_title("MACD")
                st.pyplot(fig)
    ...
    ax.axhline(y=80, color='green', linestyle='--', alpha=0.5, label='Elite (80%)')
    ax.axhline(y=60, color='orange', linestyle='--', alpha=0.5, label='Consistent (60%)')
    ax.set_ylim(0,105)
    ax.set_ylabel("Score %")
...
elif st.session_state['active_tab'] == "📝 Trading Journal":
    st.title("📝 Trading Journal")
    
    tab1, tab2, tab3 = st.tabs(["📊 Record Trade", "📓 Journal Entry", "📜 History"])
    
    with tab1:
        st.subheader("Record a New Trade")
        with st.form("trade_form"):
            col1, col2 = st.columns(2)
            with col1:
                trade_date = st.date_input("Trade Date", datetime.now())
                index_name = st.selectbox("Index", ["Nifty", "Bank Nifty", "Sensex", "BTC", "ETH"])
                trade_type = st.selectbox("Trade Type", ["Intraday", "Positional"])
                strategy = st.text_input("Strategy Employed", placeholder="e.g., ISO, Iron Condor, Credit Spread")
            with col2:
                quantity = st.number_input("Quantity", min_value=1, value=1)
                capital = st.number_input("Capital (₹)", min_value=0, value=100000)
                pnl = st.number_input("P/L Amount", value=0.0)
                rr = st.text_input("Risk/Reward Ratio", placeholder="e.g., 1:2")
            
            remarks = st.text_area("Remarks", placeholder="What worked? What didn't?")
            ...
            if submitted:
                ...
                st.success(f"Trade recorded! P/L: ₹{pnl:,.2f}")
    
    with tab2:
        st.subheader("Daily Journal Entry")
        with st.form("journal_form"):
            entry_title = st.text_input("Title", placeholder="e.g., Market Analysis for Today")
            pre_market_analysis = st.text_area("Pre-Market Analysis (9:30-9:45)", 
                                               placeholder="Range, trend, global cues...", height=150)
            morning_range = st.text_area("Morning Range Analysis", height=100)
            trading_time_analysis = st.text_area("Trading Time Analysis", height=100)
            conclusion = st.text_area("Conclusion & Learnings", height=100)
            
            # Quick checkboxes
            st.markdown("**Quick Checks:**")
            col_a, col_b, col_c = st.columns(3)
            with col_a:
                had_setup = st.checkbox("Had a setup")
                followed_rules = st.checkbox("Followed rules")
            with col_b:
                paper_trade_done = st.checkbox("Paper trade done")
                real_trade_done = st.checkbox("Real trade done")
            with col_c:
                journal_maintained = st.checkbox("Journal maintained")
                meditation_done = st.checkbox("Meditation done")
            ...
            if submitted:
                entry_data = {
                    ...
                    'trading_time_analysis': trading_time_analysis,
                    'conclusion': conclusion,
                    'had_setup': had_setup,
                    'followed_rules': followed_rules,
                    'paper_trade_done': paper_trade_done,
                    'real_trade_done': real_trade_done,
                    'journal_maintained': journal_maintained,
                    'meditation_done': meditation_done
                }
                TradingJournal.add_journal_entry(entry_data)
                st.success("Journal entry saved!")
    
    with tab3:
        st.subheader("Trade History")
        all_entries = TradingJournal.load_journal()
        trades = [e for e in all_entries if e.get('type') == 'trade']
        journals = [e for e in all_entries if e.get('type') == 'journal']
        
        if trades:
            trades_df = pd.DataFrame(trades)
            trades_df['date'] = pd.to_datetime(trades_df['date']).dt.date
            st.dataframe(trades_df[['date', 'index', 'trade_type', 'strategy', 'quantity', 'capital', 'pnl', 'remarks']], 
            ...
            col_p1.metric("Total P/L", f"₹{total_pnl:,.2f}", delta=f"{win_rate:.0f}% Win Rate")
            col_p2.metric("Total Trades", len(trades))
            col_p3.metric("Avg Trade", f"₹{total_pnl/len(trades):,.2f}" if trades else "₹0")
        else:
            st.info("No trades recorded yet.")
        
        st.subheader("Journal Entries")
        if journals:
            for journal in journals[-5:]:  # Show last 5
                st.markdown(f"""
                <div class="journal-entry">
                    <strong>📔 {journal.get('title', 'Untitled')}</strong><br>
                    <small>{journal.get('date', '')[:10]}</small>
                    <hr>
                    <strong>Pre-Market:</strong> {journal.get('pre_market_analysis', '')[:200]}...
                    <br>
                    <strong>Conclusion:</strong> {journal.get('conclusion', '')[:200]}...
                </div>
                """, unsafe_allow_html=True)
        else:
