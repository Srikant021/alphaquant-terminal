...
# -----------------------------------------------------------------------------
def plot_technical_analysis(ticker, asset_name):
    df = yf_download_retry(ticker, period="6mo", interval="1d")
    if df.empty: return None
    df = flatten_df(df)
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"{asset_name} Price", "Volume", "RSI (14)", "MACD")
    )
    ...
    if 'BBL_20_2.0' in df.columns:
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['BBM_20_2.0'], line=dict(color='white', width=0.5), name='Middle Band'), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['MACD_12_26_9'], line=dict(color='cyan', width=1.5), name='MACD'), row=4, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['MACDs_12_26_9'], line=dict(color='orange', width=1), name='Signal'), row=4, col=1)
    fig.add_trace(go.Bar(x=df.index, y=df['MACDh_12_26_9'], marker_color='gray', name='Histogram'), row=4, col=1)
...
# -----------------------------------------------------------------------------
# NEW FEATURES (Multi‑TF, Payoff, Risk, News, Backtest)
# -----------------------------------------------------------------------------
...
with st.expander("📈 Technical Analysis (Daily)"):
    fig_ta = plot_technical_analysis(ticker, asset_choice)
    if fig_ta:
        st.plotly_chart(fig_ta, use_container_width=True)
    else:
        st.warning("Could not generate technical analysis chart.")
...
st.markdown("---")
st.caption("AlphaQuant Terminal Pro · Complete Cockpit")