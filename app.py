# AlphaQuant Terminal - Unified Dashboard (Bug-Fixed)
# Fixes applied:
#   1. Ticker corrected: ^NSEi -> ^NSEI
#   2. Hurst return tuple always 4 values; caller uses consistent unpacking
#   3. hist_2y MultiIndex flattened before column access
#   4. implied_vol fetched live from ^INDIAVIX / ^VIX instead of hardcoded
#   5. Parkinson vol uses mean() instead of sum()/len() for correct annualisation
#   6. resample alias 'ME' -> 'MS' with pandas-version fallback
#   7. OI profile uses np.random.default_rng with better seed
#   8. Timestamp uses IST timezone explicitly
...
# ============================================================
# IST TIMEZONE  (Fix #8 — was using naive datetime.now())
# ============================================================
IST = timezone(timedelta(hours=5, minutes=30))


# ============================================================
# PANDAS VERSION HELPER  (Fix #6)
# ============================================================
def _month_end_alias() -> str:
    """Return the correct month-end resample alias for the installed pandas."""
...
def hurst_exponent(price_series):
    ...
    Always returns a 3-tuple so callers don't need isinstance checks.  (Fix #2)
    """
...
def compute_parkinson_vol(high, low, periods=252):
    """
    Fix #5 — use mean() so the result is independent of window length.
    variance = E[ (ln(H/L))^2 / (4 ln2) ]  then annualise.
    """
    ...
    variance = log_hl.mean() / (4 * np.log(2))       # Fix #5
...
def fetch_data(ticker, period="1y", interval="1d"):
    try:
        ...
        if data.empty:
            return None
        data = _flatten_multiindex(data)          # always flatten  (Fix #3)
        return data
...
def fetch_live_vix(market: str) -> float:
    """
    Fix #4 — fetch the actual VIX instead of using a hardcoded constant.
    Falls back to a sensible default if the ticker is unavailable.
    """
...
def create_hurst_yearly_chart(daily_1y):
    """
    Fix #2 — always returns a 4-tuple: (fig_or_None, hurst, interp, confidence).
    Caller no longer needs isinstance / len checks.
    """
...
def create_oi_profile(spot):
    """Fix #7 — use default_rng with a collision-resistant seed."""
    step  = 500 if spot > 10000 else 100 if spot > 2000 else 50
    base  = round(spot / step) * step
    strikes = np.arange(base - 8 * step, base + 9 * step, step)

    # Fix #7: seed derived from spot * 100 to avoid collisions between nearby prices
...
# Fix #3 — hist_2y MultiIndex already flattened inside fetch_data
if hist_2y is not None and not hist_2y.empty:
    close_2y = hist_2y['Close'].squeeze()
else:
    close_2y = close_1y
...
# Fix #2 — always unpack as 4-tuple; no isinstance check needed
...
tabs = st.tabs(list(TF_MAP.keys()))
for tab, (label, (period_tf, interval_tf)) in zip(tabs, TF_MAP.items()):
    with tab:
...
with lsw_col3:
    # Fix #6 — use pandas-version-safe month-end alias
...
st.markdown('<div class="section-header">Market Regime — Hurst Exponent (1 Year Daily)</div>',
            unsafe_allow_html=True)
...
with v2:
    st.markdown(f"#### Expected Move (1σ, 68% probability)  —  IV: {implied_vol:.1f}%")
